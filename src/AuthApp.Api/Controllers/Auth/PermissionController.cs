...
namespace AuthApp.Api.Controllers.Auth
{
    ...
    public class PermissionController : ControllerBase
    {
        ...
        readonly IModulePermissionRepository _modulePermissionRepository;
        ...
        public PermissionController(IPermissionRepository permissionRepository, IModulePermissionRepository modulePermissionRepository,
            ...
            IAspNetUser aspNetUser, PermissionRequirement requirement)
        {
        ...
        // GET: api/User
        [HttpGet]
        public async Task<MessageModel<PageResult<Permission>>> Get(int page = 1, string key = "")
        {
            ...
            var query = await _permissionRepository.GetAllAsync();
            var pagedViewModel = new PagedViewModel<Permission>
            {
                Query = query,
            ...
            }.AddFilter(a => a.IsDeleted != true) //&& (a.Name != null && a.Name.Contains(key)))
            .Setup();
            PageResult<Permission> pageResult = pagedViewModel.ToPageData();
            ...
            var apis = await _modulePermissionRepository.GetByConditionAsync(d => d.IsDeleted == false);
            var permissionsView = pageResult.Records;

            var permissionAll = await _permissionRepository.GetByConditionAsync(d => d.IsDeleted != true);
            ...
            pageResult.Records = permissionsView;

            #endregion


            return new MessageModel<PageResult<Permission>>()
            {
                msg = "获取成功",
        ...
        public async Task<MessageModel<List<Permission>>> GetTreeTable(int f = 0, string key = "")
        {
            ...
            var apiList = await _modulePermissionRepository.GetByConditionAsync(d => d.IsDeleted == false);
            var permissionsList = await _permissionRepository.GetByConditionAsync(d => d.IsDeleted == false);
        ...
        public async Task<MessageModel<string>> Post([FromBody] Permission permission)
        {
            ...
            permission.CreateId = _aspNetUser.ID;
            permission.CreateBy = _aspNetUser.Name;

            var addPermission = (await _permissionRepository.AddAsync(permission));
           
            if (addPermission!=null)
            {
                data.success = true;
                data.response = addPermission.Id.ObjToString();
                data.msg = "添加成功";
            }
        ...
        public async Task<MessageModel<string>> Assign([FromBody] AssignView assignView)
        {
            ...
            if (assignView.rid > 0)
            {
                ...
                var roleModulePermissions = await _roleModulePermissionRepository.GetByConditionAsync(d => d.RoleId == assignView.rid);

                var remove = roleModulePermissions.Where(d => !assignView.pids.Contains(d.PermissionId.ObjToInt())).Select(c => (object)c.Id);
                data.success &= remove.Any() ? await _roleModulePermissionRepository.DeleteByIds(remove.ToArray()) : true;
                ...
                foreach (var item in assignView.pids)
                {
                    ...
                    if (!rmpitem.Any())
                    {
                        var moduleid = (await _permissionRepository.GetByConditionAsync(p => p.Id == item)).FirstOrDefault()?.Mid;
                        ...
                        roleModulePermission.CreateId = _aspNetUser.ID;
                        roleModulePermission.CreateBy = _aspNetUser.Name;

                        var addRoleModulePermission=await _roleModulePermissionRepository.AddAsync(roleModulePermission);
                        
                        data.success &= (addRoleModulePermission != null);
        ...
        public async Task<MessageModel<PermissionTree>> GetPermissionTree(int pid = 0, bool needbtn = false)
        {
            ...
            var permissions = await _permissionRepository.GetByConditionAsync(d => d.IsDeleted == false);
        ...
        public async Task<MessageModel<NavigationBar>> GetNavigationBar(int uid)
        {
            ...
            //{
                ...
                roleIds = (await _userRoleRepository.GetByConditionAsync(d => d.IsDeleted == false && d.UserId == uid)).Select(d => d.RoleId.ObjToInt()).Distinct().ToList();
            //}
            ...
            if (uid > 0 && uid == uidInHttpcontext1)
            {
                if (roleIds.Any())
                {
                    var pids = (await _roleModulePermissionRepository.GetByConditionAsync(d => d.IsDeleted == false && roleIds.Contains(d.RoleId))).Select(d => d.PermissionId.ObjToInt()).Distinct();
                    if (pids.Any())
                    {
                        var rolePermissionMoudles = (await _permissionRepository.GetByConditionAsync(d => pids.Contains(d.Id))).OrderBy(c => c.OrderSort);
        ...
        public async Task<MessageModel<AssignShow>> GetPermissionIdByRoleId(int rid = 0)
        {
            ...
            var rmps = await _roleModulePermissionRepository.GetByConditionAsync(d => d.IsDeleted == false && d.RoleId == rid);
        ...
        public async Task<MessageModel<string>> Put([FromBody] Permission permission)
        {
            ...
            {
                var permissionUpdate = await _permissionRepository.UpdateAsync(permission);
                await _roleModulePermissionRepository.UpdateModuleId(permission.Id, permission.Mid);
        ...
        public async Task<MessageModel<string>> Delete(int id)
        {
            ...
            if (id > 0)
            {
                var userDetail = await _permissionRepository.GetAsync(id);
                userDetail.IsDeleted = true;
                var permissionUpdate= await _permissionRepository.UpdateAsync(userDetail);
                if (permissionUpdate!=null)
                {
                    data.success = true;
                    data.msg = "删除成功";
